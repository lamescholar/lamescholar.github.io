**FB2**<br/><br/>

Чтение:

Компьютер:

Calibre - <https://calibre-ebook.com/download_windows>

Esc - меню. Alt+Left - возврат к предыдущему положению в книге. Обычно подходит размер шрифта 20 px, иногда 26 px. Настройки. Макет страниц->Альбомная: 1. Поведение при прокрутке->Разное->Показать полосу прокрутки. Разное->Помнить последний размер и положение окна.

Телефон:

Pocketbook<br/><br/>

Распознание:

ABBYY Finereader PDF 15 - <https://rutracker.org/forum/viewtopic.php?t=6040898><br/><br/>

Создание:

https://www.youtube.com/@andreilazarov8881/videos

FictionBook Editor 2.6.6:

<https://download.cnet.com/MSXML-4-0-Service-Pack-2-Microsoft-XML-Core-Services/3000-2070_4-10726922.html>

<https://code.google.com/archive/p/fictionbookeditor/downloads>

При расплывчатом тексте те же настройки совместимости, что и для WinDjView.<br/><br/>

Конвертация:

LibreOffice - <https://www.libreoffice.org/download/download/?lang=ru>

OOoFBTools - <https://sourceforge.net/projects/fbtools/>

DOCX->FB2

Calibre - <https://calibre-ebook.com/download_windows>

EPUB->DOCX

FB2->DOCX

HTM->DOCX

HTM->EPUB<br/><br/>

Где опубликовать книгу:

<http://flibusta.is/a><br/><br/>

Информация:

<https://fb2-reader.ru/>