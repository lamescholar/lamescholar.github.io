...
С помощью этого скрипта можно скачать любую книгу с Internet Archive. Cкрипт скачивает картинки страниц. Ниже есть три варианта, как ты можешь конвертировать картинки в PDF или DjVu.
<br><br>
...
Если сканы тёмные, а текст блеклый, используй инструменты цветокоррекции Contrast и Gamma correction в программе IrfanView . Я использую следующую сетку значений (первый столбик - Contrast, второй - Gamma correction):
...
Нужно экспериментировать с разными комбинациями - Shift+G. Визуально Contrast жирнит текст и повышает яркость фона. Gamma correction подсветляет скан, но выцветляет текст.

Полезные комбинации:<br>
70 2.00 (есть зеленоватый или оранжевый оттенок),<br>
50/70 (текст блеклый),<br>
50 0.75 (скан довольно светлый, текст блеклый),<br>
0.62 (скан очень светлый),<br>
90 6.99 (скан очень тёмный).

Если остался зеленоватый или оранжевый оттенок, понизь насыщенность цветов скана. Для этого используй инструмент Saturation. Значение: -100/-150.
...
Помимо инструментов цветокоррекции, я постоянно использую инструмент Sharpen против размытого текста. Оптимальное значение: 30. В экстремальных случаях: 60.

Скрипт скачивает картинки без определённого DPI. Из-за этого могут быть проблемы при создании файла. Чтобы установить DPI, используй IrfanView.
...
Use advanced options (for bulk resize...)->Advanced->Save new DPI value: 300 или 600. Если ширина картинок меньше 2000 - 300 DPI, больше 2000 - 600 DPI. Вводишь значения Gamma correction, Constrast, Saturation, Sharpen, если необходимо.
